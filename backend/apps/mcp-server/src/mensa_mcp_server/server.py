...
def make_openmensa_client() -> OpenMensaClient:
    ...
    # OpenMensa SDK uses a sync requests.Session internally. We instrument at the
    # session layer to count external calls without changing the SDK API.
    ...
    if isinstance(sess, requests.Session):
        ...
        def instrumented_request(method, url, *args, **kwargs):  # type: ignore[no-untyped-def]
            ...
            try:
                ...
                metrics.inc_labeled("openmensa.http.error_type_total", type(e).__name__)
                raise
